...
export class CvBootstrapComponent {
	...
	async downloadPDF() {
		...
		const templateData = {
			...
			skills: { technical: cv.skills.map(s => ({ name: s, version: '', experience: '' })), soft: [] },
			education: [],
			projects: cv.projects.map(p => ({
				name: p.name, description: p.description,
				technologies: p.technologies ? p.technologies.split(',').map((t: string) => t.trim()) : [], url: ''
			})),
			certifications: cv.certifications.map(c => ({ name: c.name, issuer: c.issuer, date: '', credential_id: '' }))
		};
		const html = creativeTemplate(templateData as any);
		const filename = this.getUserName() || 'Resume';
		try {
